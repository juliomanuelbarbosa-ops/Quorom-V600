...
import { 
  Gamepad2, Search, ExternalLink, Cpu, 
  Monitor, Smartphone, Database, Zap, 
  ShieldCheck, Info, Filter, Star, 
  Terminal, Layers, MonitorPlay
} from 'lucide-react';
...
const EMULATOR_DATA: EmulatorNode[] = [
  { name: "Ryujinx", repo: "Ryujinx/Ryujinx", platform: 'Nintendo', type: 'Home', description: "Experimental Nintendo Switch emulator written in C#.", status: 'ACTIVE' },
  { name: "Dolphin", repo: "dolphin-emu/dolphin", platform: 'Nintendo', type: 'Home', description: "GameCube and Wii emulator. The gold standard for performance and accuracy.", status: 'STABLE' },
  { name: "RPCS3", repo: "RPCS3/rpcs3", platform: 'Sony', type: 'Home', description: "The world's first open-source PlayStation 3 emulator/debugger.", status: 'ACTIVE' },
  { name: "PCSX2", repo: "PCSX2/pcsx2", platform: 'Sony', type: 'Home', description: "PlayStation 2 emulator capable of running 99% of the original library.", status: 'STABLE' },
  { name: "DuckStation", repo: "stenzek/duckstation", platform: 'Sony', type: 'Home', description: "Fast PlayStation 1 emulator focusing on playability and speed.", status: 'STABLE' },
  { name: "mGBA", repo: "mgba-emu/mgba", platform: 'Nintendo', type: 'Handheld', description: "New generation of Game Boy Advance emulator, optimized for accuracy.", status: 'STABLE' },
  { name: "Citra", repo: "citra-emu/citra", platform: 'Nintendo', type: 'Handheld', description: "Experimental Nintendo 3DS emulator.", status: 'LEGACY' },
  { name: "PPSSPP", repo: "hrydgard/ppsspp", platform: 'Sony', type: 'Handheld', description: "Cross-platform PSP emulator with high resolution rendering.", status: 'STABLE' },
  { name: "Flycast", repo: "flyinghead/flycast", platform: 'Sega', type: 'Home', description: "Multi-platform Sega Dreamcast, Naomi and Atomiswave emulator.", status: 'ACTIVE' },
  { name: "MAME", repo: "mamedev/mame", platform: 'Arcade', type: 'Desktop', description: "Multiple Arcade Machine Emulator. Preservation substrate for arcade history.", status: 'STABLE' },
  { name: "RetroArch", repo: "libretro/RetroArch", platform: 'Multi', type: 'Desktop', description: "The definitive frontend for emulators, game engines and media players.", status: 'STABLE' },
  { name: "Xenia", repo: "xenia-project/xenia", platform: 'Microsoft', type: 'Home', description: "Experimental Xbox 360 Research Emulator.", status: 'ACTIVE' }
];
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-20 h-full flex flex-col">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-orange-500/20 pb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <a href="https://github.com/DerekTurtleRoe/awesome-emulators" target="_blank" rel="noopener noreferrer" className="text-orange-900 hover:text-orange-400 transition-colors">
          ...
          {platforms.map(p => (
            <button 
              ...
              className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap border ${
                filter === p ? 'bg-orange-600 text-white border-orange-400 shadow-lg' : 'bg-slate-900/40 text-orange-900 border-orange-500/10 hover:border-orange-500/40'
              }`}
            >
              {p}
            </button>
            ...
            >
              ...
              <div className="flex justify-between items-start mb-4">
                <span className={`px-2.5 py-1 rounded text-[8px] font-black uppercase tracking-widest border ${
                  emu.type === 'Handheld' ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' :
                  emu.type === 'Home' ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20' :
                ...
                <div className="flex gap-2">
                  <span className={`text-[8px] font-black px-1.5 py-0.5 rounded border ${
                    emu.status === 'STABLE' ? 'bg-green-500/10 text-green-500 border-green-500/20' :
                    emu.status === 'ACTIVE' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' :
              ...
              <div className="space-y-4 pt-4 border-t border-orange-500/5">
                <div className="flex items-center justify-between">
                   <div className="flex items-center gap-2">
                      ...
                      <span className="text-[9px] font-black text-orange-800 uppercase tracking-tighter">{emu.platform} Core</span>
                   </div>
                   <a 
      ...
      <div className="p-8 bg-orange-500/5 border border-orange-500/10 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-8 mt-auto">
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-1">
            <h4 className="text-xs font-black text-orange-400 uppercase tracking-widest font-mono">Dynamic Recompiler Protocol</h4>
            <p className="text-[10px] text-orange-900 font-bold uppercase leading-relaxed max-w-xl italic">
              "Abstractions must be complete. Every register of the original silicon is a coordinate in the timeline of computational history."
            </p>
          </div>
        </div>
        <div className="flex items-center gap-6">
