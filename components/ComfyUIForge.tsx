...
const ComfyUIForge: React.FC = () => {
  ...
  const [prompt, setPrompt] = useState(
    'A digital painting of a cyberpunk city floating in a void of neon gas, hyper-realistic, 8k'
  );
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-indigo-500/20 pb-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <a
            ...
            className="text-indigo-900 hover:text-indigo-400 transition-colors"
          >
                ...
                className={`absolute w-44 glass-panel rounded-xl border ${
                  node.status === 'running'
                    ...
                    : node.status === 'complete'
                      ? 'border-indigo-900/50 opacity-80'
                      : 'border-indigo-500/10'
                } overflow-hidden`}
              >
                <div
                  className={`p-2 border-b ${node.status === 'running' ? 'bg-indigo-500 text-slate-950' : 'bg-slate-900/50 text-indigo-500'} flex justify-between items-center transition-colors`}
                >
        ...
        <div className="lg:col-span-4 flex flex-col gap-6 h-full overflow-hidden">
          <div className="glass-panel p-6 rounded-2xl border border-indigo-500/20 bg-slate-900/30 flex flex-col gap-6">
            <div className="space-y-4">
              <label htmlFor="positive-latent" className="text-[9px] text-indigo-900 font-black uppercase tracking-widest flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <label htmlFor="steps-input" className="text-[8px] text-indigo-900 font-black uppercase">Steps</label>
                <input
                  id="steps-input"
                  type="number"
                  defaultValue={20}
                  className="w-full bg-slate-950 border border-indigo-500/10 rounded p-2 text-[10px] text-indigo-400"
                />
              </div>
              <div className="space-y-1">
                <label htmlFor="cfg-scale-input" className="text-[8px] text-indigo-900 font-black uppercase">CFG Scale</label>
                <input
                  id="cfg-scale-input"
                  type="number"
                  defaultValue={8.0}
