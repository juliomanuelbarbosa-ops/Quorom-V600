...
import { 
  Wallet, Shield, Zap, ArrowRight, Copy, Terminal as TerminalIcon, 
  Cpu, HardDrive, Globe, RefreshCcw, Check, Loader2, AlertCircle,
  Link as LinkIcon, ExternalLink, ChevronDown
} from 'lucide-react';
...
const CryptoHub: React.FC<CryptoHubProps> = ({ mode }) => {
  ...
  const [balances, setBalances] = useState<ChainBalance[]>([
    { network: 'Ethereum', symbol: 'ETH', amount: 12.45, fiatValue: 32450.12, status: 'synced', color: 'text-cyan-400' },
    { network: 'Solana', symbol: 'SOL', amount: 452.1, fiatValue: 68412.33, status: 'synced', color: 'text-purple-400' },
    { network: 'Polygon', symbol: 'MATIC', amount: 8902.5, fiatValue: 4120.45, status: 'synced', color: 'text-indigo-400' }
  ]);
    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-cyan-800 text-xs uppercase tracking-[0.4em] font-bold mt-1">Non-Custodial Multi-Chain Node</p>
        </div>
        ...
        {walletState === 'disconnected' && (
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <p className="text-cyan-800 text-xs font-bold uppercase tracking-widest max-w-sm mx-auto">Authorize your cryptographic signature to access local and global subnets.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
              ...
              >
                ...
                <div className="text-[10px] font-black uppercase text-cyan-800 group-hover:text-cyan-400">Browser Wallet</div>
              </button>
              ...
              >
                ...
                <div className="text-[10px] font-black uppercase text-cyan-800 group-hover:text-cyan-400">Ledger Nano</div>
              </button>
              ...
              >
                ...
                <div className="text-[10px] font-black uppercase text-cyan-800 group-hover:text-cyan-400">Trezor One</div>
              </button>
            </div>
          </motion.div>
        )}
          ...
          >
            <div className="relative w-32 h-32 mx-auto">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                {connectMethod === 'browser' ? <LinkIcon className="text-cyan-400 animate-pulse" size={32} /> : <HardDrive className="text-cyan-400 animate-pulse" size={32} />}
              </div>
            </div>
            <div className="space-y-2">
              ...
              <p className="text-[10px] text-cyan-900 font-bold uppercase tracking-widest">
                {connectMethod === 'browser' ? 'Waiting for injected provider authorization' : 'Scanning WebHID interface for compatible device'}
              </p>
            </div>
          </motion.div>
        )}
            ...
            <div className="lg:col-span-2 space-y-8">
              <div className="glass-panel p-8 rounded-2xl border border-cyan-500/20 space-y-8">
                  ...
                  {balances.map((balance) => (
                    <motion.div 
                      ...
                      className={`p-6 rounded-2xl border transition-all cursor-pointer ${
                        activeChain === balance.network ? 'bg-cyan-500/10 border-cyan-500 shadow-[0_0_20px_rgba(6,182,212,0.1)]' : 'bg-slate-900/50 border-cyan-500/10 hover:border-cyan-500/30'
                      }`}
                      onClick={() => setActiveChain(balance.network)}
                    >
                      <div className="flex justify-between items-start mb-4">
                        <span className="text-[10px] font-black text-cyan-900 uppercase tracking-widest">{balance.network}</span>
                      ...
                      <div className="space-y-1">
                        <div className="text-2xl font-black text-slate-100">{balance.amount.toFixed(2)} <span className={balance.color}>{balance.symbol}</span></div>
                        <div className="text-[10px] text-cyan-900 font-bold uppercase tracking-widest">≈ ${balance.fiatValue.toLocaleString()}</div>
                      </div>
                    </motion.div>
                  ))}
                </div>
                ...
                <div className="p-6 bg-slate-950/50 rounded-2xl border border-cyan-500/10 space-y-6">
                   <div className="flex justify-between items-center">
                      <div className="text-xs font-black text-cyan-400 uppercase tracking-widest">Active Chain: {activeChain}</div>
                      <div className="flex space-x-2">
                         <div className="px-2 py-0.5 bg-green-500/10 border border-green-500/20 rounded text-[8px] font-black text-green-500 uppercase tracking-widest">Healthy</div>
                         <div className="px-2 py-0.5 bg-slate-800 border border-cyan-500/10 rounded text-[8px] font-black text-cyan-800 uppercase tracking-widest">Latency: 12ms</div>
                      </div>
                   </div>
              ...
              <div className="glass-panel p-8 rounded-2xl border border-cyan-500/20 space-y-6 h-full flex flex-col">
                ...
                {mode === 'creator' ? (
                  <div className="space-y-6 flex-1 flex flex-col">
                    <div className="space-y-4">
                      <div>
                        <label className="text-[10px] text-cyan-900 font-bold uppercase mb-2 block tracking-widest">Token Name</label>
                        <input 
                          type="text" 
                      ...
                      <div>
                        <label className="text-[10px] text-cyan-900 font-bold uppercase mb-2 block tracking-widest">Symbol</label>
                        <input 
                          type="text" 
                          className="w-full bg-slate-950 border border-cyan-500/20 rounded-xl p-4 text-cyan-400 focus:outline-none focus:border-cyan-500 text-sm font-bold tracking-wider"
                          placeholder="NMX"
                  ...
                  <div className="space-y-4">
                     <div className="p-4 bg-cyan-500/5 border border-cyan-500/10 rounded-xl flex items-start space-x-3">
                        ...
                        <p className="text-[10px] text-cyan-800 font-bold uppercase tracking-wide leading-relaxed">
                          Sovereign Wallet mode restricts smart contract deployment to ensure high-entropy security for stored assets.
                        </p>
                     </div>
                     <div className="space-y-2">
                        {[1, 2].map(i => (
                           <div key={i} className="flex items-center justify-between p-3 bg-slate-900/50 rounded-lg border border-cyan-500/10">
                              <div className="flex items-center space-x-2">
                                 <TerminalIcon size={12} className="text-cyan-800" />
                                 <span className="text-[10px] text-slate-400 font-mono">0x{Math.random().toString(16).slice(2, 8)}... Tx</span>
                              </div>
