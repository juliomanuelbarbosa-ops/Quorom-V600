...
const AutoGPTHub: React.FC = () => {
  ...
  const simulateLoop = () => {
    ...
    const steps: Thought[] = [
      ...
      {
        type: 'reasoning',
        text: 'Target requires multi-step decomposition. Accessing local knowledge base for substrate verification.',
        timestamp: '0.450s',
      },
      {
        type: 'plan',
        text: '1. Scan mesh for vulnerabilities\n2. Deploy sub-agent for data extraction\n3. Synthesize final report',
        timestamp: '1.200s',
      },
      {
        type: 'criticism',
        text: 'Initial plan lacks post-quantum encryption check. High risk of signal interception. Amending protocol.',
        timestamp: '2.100s',
      },
      {
        type: 'action',
        text: 'Executing secure handshake with Node_04X. Initializing subprocess...',
        timestamp: '3.500s',
      },
    ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 flex-1 overflow-hidden">
        ...
        <div className="space-y-6 flex flex-col">
          <div className="glass-panel p-6 rounded-2xl border border-cyan-500/20 space-y-6">
            <div className="space-y-4">
              <label htmlFor="mission-objective" className="text-[10px] text-cyan-900 font-black uppercase tracking-widest flex items-center gap-2">
        ...
        <div className="lg:col-span-3 flex flex-col overflow-hidden">
          <div className="glass-panel rounded-3xl border border-cyan-500/20 flex flex-col flex-1 bg-slate-950/40 overflow-hidden">
            ...
            <div ref={scrollRef} className="flex-1 p-6 overflow-y-auto custom-scrollbar space-y-4 font-mono">
              <AnimatePresence>
                {thoughts.length === 0 ? (
                ...
                {isRunning && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-3 p-4">
                    <Loader2 size={14} className="animate-spin text-cyan-500" />
                    <span className="text-[10px] text-cyan-900 font-black uppercase tracking-widest animate-pulse">
                      Processing Cognitive Cycle...
                    </span>
                  </motion.div>
                )}
              </AnimatePresence>
