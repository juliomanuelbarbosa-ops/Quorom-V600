...
const GeoVigilanceHub: React.FC = () => {
  ...
  const fetchGeoData = async () => {
    ...
    try {
      ...
      allEvents.filter(e => e.severity === 'CRITICAL').forEach(e => {
        addIntelBrief({
          id: `geo-${e.id}`,
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'EARTHQUAKE': return <Waves className="text-amber-500" />;
      case 'STORM': return <Wind className="text-cyan-400" />;
      case 'CONFLICT': return <Flame className="text-red-500" />;
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-20 h-full flex flex-col">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-red-500/20 pb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            {isLoading ? <Loader2 size={14} className="animate-spin text-red-500" /> : <Zap size={14} className="text-red-500" />}
          </button>
        </div>
      </div>
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <div className={`text-[8px] font-black px-1.5 py-0.5 rounded border ${
                        event.severity === 'CRITICAL' ? 'bg-red-500 text-slate-950 border-red-400' : 'bg-slate-950 text-red-500 border-red-900'
                      }`}>
                        {event.severity}
                      </div>
                    </div>
        ...
        <div className="lg:col-span-8 flex flex-col gap-6 overflow-hidden">
          <div className="glass-panel flex-1 rounded-3xl border border-red-500/20 bg-slate-950/40 relative overflow-hidden">
            ...
            <div className="absolute inset-0 opacity-5 pointer-events-none" 
                 style={{ backgroundImage: 'radial-gradient(circle, #ef4444 1px, transparent 1px)', backgroundSize: '40px 40px' }}></div>
                  ...
                  {events.map((event) => (
                    ...
                    >
                      <div className={`w-4 h-4 rounded-full border-2 animate-pulse ${
                        event.severity === 'CRITICAL' ? 'bg-red-500 border-red-300 shadow-[0_0_15px_#ef4444]' : 'bg-amber-500 border-amber-300'
                      }`}></div>
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="text-xl font-black text-red-500 uppercase tracking-tighter">{selectedEvent.title}</h3>
                        <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">{selectedEvent.location}</p>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex gap-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-red-900 font-black">{selectedEvent.lat.toFixed(2)}, {selectedEvent.lng.toFixed(2)}</span>
                      </div>
                      <div className="flex items-center gap-2">
              ...
              <div className="p-3 bg-slate-950/80 border border-red-500/20 rounded-xl backdrop-blur-md">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="w-20 h-12 bg-slate-900 rounded border border-red-500/10 overflow-hidden relative group cursor-pointer">
                    <img src="https://picsum.photos/seed/cam1/200/120" className="w-full h-full object-cover opacity-50 group-hover:opacity-100 transition-opacity" referrerPolicy="no-referrer" />
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse"></div>
                    </div>
                  </div>
                  <div className="w-20 h-12 bg-slate-900 rounded border border-red-500/10 overflow-hidden relative group cursor-pointer">
                    <img src="https://picsum.photos/seed/cam2/200/120" className="w-full h-full object-cover opacity-50 group-hover:opacity-100 transition-opacity" referrerPolicy="no-referrer" />
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse"></div>
                    </div>
                  </div>
