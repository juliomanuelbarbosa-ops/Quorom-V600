...
import { 
  Network, Search, ExternalLink, Zap, 
  Database, Globe, Terminal, Shield, 
  Cpu, Code2, Link as LinkIcon, Box,
  MessageSquare, Layout, Filter, Sparkles
} from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-20 h-full flex flex-col">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-cyan-500/20 pb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <a href="https://github.com/punkpeye/awesome-mcp-servers" target="_blank" rel="noopener noreferrer" className="text-cyan-900 hover:text-cyan-400 transition-colors">
          ...
          {categories.map(cat => (
            <button 
              ...
              className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap border ${
                filter === cat ? 'bg-cyan-500 text-slate-950 border-cyan-400 shadow-lg' : 'bg-slate-900/40 text-cyan-900 border-cyan-500/10 hover:border-cyan-500/40'
              }`}
            >
              {cat}
            </button>
              ...
              <div className="space-y-4">
                <div className="flex flex-wrap gap-1.5">
                  {server.capabilities.map(cap => (
                    <span key={cap} className="text-[7px] font-black text-cyan-900 uppercase border border-cyan-500/10 px-1.5 py-0.5 rounded bg-slate-950/50">
                      {cap}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-cyan-500/5">
                   <div className="flex items-center gap-2">
                      ...
                      <span className="text-[9px] font-black text-cyan-800 uppercase tracking-tighter">Auth: {server.author}</span>
                   </div>
      ...
      <div className="p-8 bg-cyan-500/5 border border-cyan-500/10 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-8 mt-auto">
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-1">
            <h4 className="text-xs font-black text-cyan-400 uppercase tracking-widest font-mono">Model Context Protocol (MCP)</h4>
            <p className="text-[10px] text-cyan-900 font-bold uppercase leading-relaxed max-w-xl italic">
              "A standardized communication substrate allowing neural models to securely interact with the physical and digital world through managed toolsets."
            </p>
          </div>
        </div>
        <div className="flex items-center gap-4">
