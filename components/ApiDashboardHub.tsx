    ...
    <div className="max-w-6xl mx-auto space-y-8 h-full flex flex-col">
      <div className="flex items-center justify-between border-b border-cyan-500/20 pb-4">
        <div>
          ...
          <p className="text-cyan-800 text-xs uppercase tracking-[0.2em] mt-1">Real-time Environmental Synchronization</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-1 overflow-auto">
        ...
        <div className="md:col-span-2 glass-panel rounded-2xl border border-cyan-500/20 overflow-hidden flex flex-col relative group">
          ...
          <div className="flex-1 bg-slate-950 relative min-h-[300px]">
             {nasa ? (
               <>
                 <img src={nasa.url} alt={nasa.title} className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:opacity-80 transition-opacity duration-700" />
                 <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent to-transparent" />
                 <div className="absolute bottom-0 left-0 right-0 p-8 space-y-2">
                    <h3 className="text-xl font-black text-cyan-400 uppercase tracking-tighter">{nasa.title}</h3>
                    <p className="text-[10px] text-slate-400 line-clamp-3 leading-relaxed uppercase tracking-wider">{nasa.explanation}</p>
                 </div>
               </>
             ) : (
          ...
          <div className="glass-panel p-6 rounded-2xl border border-cyan-500/20 bg-slate-900/40 relative overflow-hidden h-fit">
            ...
            {quote ? (
              <div className="space-y-4">
                ...
                <div className="text-right text-[10px] font-bold text-cyan-500 tracking-widest uppercase">— {quote.a}</div>
              </div>
            ) : (
